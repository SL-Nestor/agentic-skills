...
- [ ] 判決是明確的二元結果：**PASS** 或 **FAIL**（無「有條件 PASS」或中間態）
- [ ] 判決基於獨立驗證，未僅依賴 Code Review Report 的結論
- [ ] 所有 High/Critical STRIDE 威脅均有對應緩解措施，且已通過測試驗證
- [ ] 認證與授權：所有端點有適當保護，無 IDOR 漏洞
- [ ] 資料安全：傳輸中與靜態敏感資料已加密
- [ ] 無 hardcode secret 或憑證（已搜尋整個程式碼庫）
- [ ] 日誌中無敏感資料洩漏
- [ ] 所有資安負面測試已通過
- [ ] Code Review 的 🔴 High 問題已全部修正
- [ ] FAIL 判決時：阻斷原因已具體列出每個失敗項目
- [ ] 若人類 override FAIL 判決：決策依據已記錄於 `AGENT_HANDOFF.md` 的 `Cross-Validation Conflict Log`
- [ ] Gate D 已執行並等待人類確認
