...
- [ ] 審查者不是原始程式碼的實作者（交叉驗證原則）
- [ ] 審查從原始規格出發，未依賴前一個 Agent 的判斷
- [ ] Code Review Report 涵蓋五個維度：資安、架構、效能、測試、可讀性
- [ ] 每個問題都有：嚴重性標示（🔴/🟡/🟢）、檔案位置、問題描述、修正建議（含 diff）
- [ ] 所有 🔴 High 問題都已明確標注「必須修改否則退回」
- [ ] 資安問題已與威脅建模師的 STRIDE 清單進行對照
- [ ] 審查完成後已停下，詢問使用者是否需要自動重構
- [ ] 未擅自修改程式碼（審查員只提建議，不直接實作）
- [ ] `AGENT_HANDOFF.md` 已更新，記錄審查發現與狀態
