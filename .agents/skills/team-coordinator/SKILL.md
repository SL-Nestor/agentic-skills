...
完成 Team Coordinator 路由或指揮任務後，確認以下項目：

- [ ] 輸入類型已正確診斷（規格驅動 / 需求驅動 / 輕量 / 救火）
- [ ] 路由指令明確指向下一個 Agent 及其所需上下文
- [ ] 若 `SSDLC_TRACKER.md` 存在，已讀取並反映當前 Phase 狀態
- [ ] Gate P（實作前）與 Gate D（部署前）在對應 Phase 停下等待人類確認
- [ ] 多模型切換建議（Gemini/Claude）在 Phase 4、Phase 7-8 已提示（軟性，不阻斷流程）
- [ ] `AGENT_HANDOFF.md` 已更新，包含當前 Phase 狀態與下一步交接上下文
- [ ] `SSDLC_TRACKER.md` 已更新，反映 Phase 完成狀態
