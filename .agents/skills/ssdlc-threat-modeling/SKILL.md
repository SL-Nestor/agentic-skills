...
- [ ] STRIDE 六個類別（Spoofing / Tampering / Repudiation / Information Disclosure / DoS / Elevation of Privilege）均已分析
- [ ] 每條信任邊界都有對應的威脅分析（包含內部服務之間的通訊）
- [ ] 威脅登記表已完成，每個威脅有：ID、STRIDE 類型、描述、嚴重性、緩解措施、狀態
- [ ] 所有 High/Critical 威脅都有具體緩解措施（非「待處理」）
- [ ] 嚴重性評估基於業務影響（資料損失、法規違規、營收影響），非僅技術可利用性
- [ ] 威脅建模在實作**開始之前**完成（非事後補做）
- [ ] 安全需求已注入 `tasks.md`，成為 `$implementer` 的強制任務
- [ ] 錯誤路徑、逾時路徑、重試行為均已建模（非僅 Happy Path）
- [ ] 部署目標（App Service 設定、網路規則）已納入威脅範疇
- [ ] `AGENT_HANDOFF.md` 已更新，`next_agent` 設為 `implementer`
