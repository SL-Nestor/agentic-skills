...
- [ ] Mermaid 架構圖已產出，清楚標示系統邊界與元件關係
- [ ] 所有信任邊界（Trust Boundaries）已識別並標記
- [ ] 外部依賴清單已列出（第三方 API、資料庫、訊息佇列等）
- [ ] `tasks.md` 使用 BDD 格式，每個情境包含 Given / When / Then
- [ ] OpenAPI 合約草稿（如適用）已產出
- [ ] Gate P 已執行：輸出檢查清單並停下等待人類確認
- [ ] 架構決策已記錄（選擇理由與被排除的替代方案）
- [ ] `AGENT_HANDOFF.md` 已更新，`next_agent` 設為 `threat-modeler`
- [ ] 需求變更時：先更新規格文件 → 寫 Changelog → 再更新程式碼
