...
### 1️⃣7️⃣ `$stack-advisor` (技術棧選擇面試官)
當規格書沒有明確指定該用何種前端技術時，這個指令會觸發「反轉面試 (Inversion)」。AI 會主動向你提問產品特性（SEO、登入、互動性），據此判定要採用 Type A (Vite) 還是 Type B (Next.js) 的標準架構。
> **操作範例：** `$stack-advisor 專案準備開工，但我不知道該用 Vite 還是 Next.js？`

### 1️⃣8️⃣ `$gemini-api-dev` (Gemini API 官方串接專家)
這是來自 Google 官方的 API 開發指南技能包，如果你要在系統內介接 Gemini 的能力，輸入這個指令能讓程式碼的產出正確率從 87% 大幅提升到 96%，解決 AI 模型本身文件過舊的問題。
> **操作範例：** `$gemini-api-dev 幫我用 C# .NET 寫一段呼叫 Gemini 1.5 Pro 的串接程式。`

### 1️⃣9️⃣ `$test-auditor` (測試審計與報告專家)
專門負責執行嚴格的端對端測試。它會要求你選擇測試範圍（前/後端）和環境（開發區/測試區），並且強迫對 API 紀錄 Request/Response，對前端錄製截圖與輸入狀態，最終在 `測試紀錄/YYYYMMDD/` 產出完美的 Markdown 報告。
> **操作範例：** `$test-auditor 幫我針對剛剛寫好的登入功能，在測試環境執行後端測試紀錄。`

---
## 🎨 視覺與前端工程兵工廠 (Stitch UI/UX 系列)

### 2️⃣0️⃣ `$stitch-design` (UI/UX 生成總控大師)
用來直接召喚 Stitch MCP 伺服器，將你模糊的需求文字轉化為專業的 UI 介面，並產生 `DESIGN.md`。從「想點子」到「畫出畫面」一氣呵成。
> **操作範例：** `$stitch-design 幫我生出一個 SaaS 平台的分析儀表板畫面，主色調為深邃藍。`

### 2️⃣1️⃣ `$stitch-loop` (多頁面網站一鍵生成)
當你需要一次處理多個相連的頁面時，它能從單一 Prompt 自動切分任務，自動幫你生成一個完整的跨頁網站結構。
> **操作範例：** `$stitch-loop 幫我生成一整個電子商務網站的框架，包含首頁、商品頁跟結帳頁。`

### 2️⃣2️⃣ `$react-components` (畫面轉 React 程式碼專家)
把 Stitch 剛剛畫出來的高保真度 UI 設計圖，無縫轉換成帶有正確 Design Tokens 且語法合規的 React 元件。
> **操作範例：** `$react-components 請把剛剛完成的儀表板設計，轉換成可用的 React 組件。`

### 2️⃣3️⃣ `$shadcn-ui` (Shadcn/UI 基礎設施專家)
當你需要建立前端全域性元件庫時觸發。它是整合與客製化 Shadcn UI 元件的權威專家，幫助你少走所有切版彎路。
> **操作範例：** `$shadcn-ui 幫我在目前的專案裡正確安裝與配置 DataTable, Dialog 和 Button 元件。`

---
## ⚡ 進階自動化與維護專家 (Advanced Automation)

### 2️⃣4️⃣ `$cicd-builder` (管線部署架構師)
當準備要上線或配置自動化測試時觸發。幫助你產生帶有安全憑證管理、多環境分層、甚至藍綠部署的 GitHub Actions 或 GitLab CI/CD YAML 檔。
> **操作範例：** `$cicd-builder 請幫我寫一份發布這個 .NET API 到 Azure 的 GitHub Actions。`

### 2️⃣5️⃣ `$i18n-agent` (多國語系翻譯機)
當專案需支援多語系時，它能自動抽離寫死在畫面中的中文，換成 Semantic Keys，並為你產出中/英/日等對照 JSON 字典檔。
> **操作範例：** `$i18n-agent 幫我把這個 Header 元件裡的所有寫死文字抽換成 react-i18next 的寫法，並產生 json 字典。`

### 2️⃣6️⃣ `$a11y-seo-auditor` (無障礙與 SEO 稽核員)
專門抓出會讓搜尋引擎扣分或螢幕閱讀器無法讀取的 UI 缺陷（如缺少 alt 的圖片、對比度不足的按鈕），並自動修正。
> **操作範例：** `$a11y-seo-auditor 幫我 review 首頁的程式碼，確保能拿到 Lighthouse 100 分。`

### 2️⃣7️⃣ `$api-documenter` (文件與假資料生成大師)
解決前/後端依賴的痛點。根據寫好的後端程式碼自動填補 Swagger 完整註解、產生直接可用的 Postman Collection 檔，或是生成超逼真的假資料 Seeding 腳本。
> **操作範例：** `$api-documenter 請幫我為剛剛寫好的 Users 功能生成 Postman 匯入檔，以及 50 筆逼真的假資料。`

---

### 2️⃣8️⃣ `$meta-skill` (技能創造者)
